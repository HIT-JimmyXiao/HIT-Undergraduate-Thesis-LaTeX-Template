...
%     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifhitusewindowsfontset
\IfFileExists{C:/Windows/Fonts/simsun.ttc}{\hitusewindowsfontsettrue}{\hitusewindowsfontsetfalse}
\ifdefined\hitforcefandol
  \hitusewindowsfontsetfalse
\fi
\ifhitusewindowsfontset
...
\tableofcontents %目录
\mainmatter
\include{body/bachelor_sample}
\backmatter
\include{back/conclusion}   % 结论
\bibliographystyle{hithesis}
\bibliography{reference} % 参考文献
% 新版本科毕业论文（设计）书写范例不默认包含“攻读学士学位期间取得创新性成果”页；
% 如学院另有要求，可取消下一行注释。
%\include{back/publications}    % 攻读学士学位期间取得创新性成果
\authorization             % 原创性声明和使用权限
